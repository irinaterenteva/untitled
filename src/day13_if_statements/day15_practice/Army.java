...
public class Army {
    ...
    public static void main(String[] args) {
        ...
        boolean hasDipoma = input.nextBoolean();
        System.out.println("How old are you?");
        int age = input.nextInt();

        if ((isCitizen || isResident) && hasDipoma && (age >= 18 && age <= 35)) {
            System.out.println("You are qualified for the Army"); // srting = "You are qulified for the Army";

        } else {
            System.out.println("You are not qualified");
            if (!isCitizen && !isResident) { //!(Citizen||isResident
